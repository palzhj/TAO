...
def get_parser():
    ...
    parser.add_argument("--nseq", type=int, default=3, help='sequences to be processed')
    ...
    parser.add_argument("--output", default="output.root", help="specify output filename")
    return parser
...
for nvol in range(1):
    ...
    voltage = 5 + nvol*10
    afg.setOutput(on=0)
    afg.setFrequency(10000)
    afg.setPluse(2000)
    afg.setVoltage(voltage)
    afg.setEdgeLeadin(3.0)
    afg.setEdgeTrailing(3.0)
    #afg.setOutput(on=1)

    output = TFile("vol"+str(voltage)+args.output, "recreate")
