...
void TGui::CopyChanInfo(PattExp::copyset &cset){
	...
	for (short int srcparN=0; srcparN<thisConfig->GetNParameters(); srcparN++){
		printf("src=%s\n",thisConfig->GetParName(srcparN));
		int curr_chan;
	        if(sscanf(thisConfig->GetParName(srcparN),"channel%d/%*s",&curr_chan)!=1){
	                continue;
	        }
		if(curr_chan > 35 || curr_chan < 0)
			continue;
		if(curr_chan>scope.channel)
			break;
		if(curr_chan==scope.channel){
			printf("src=%s\n",thisConfig->GetParName(srcparN));
			printf(" -- > ch=%d (%d)\n",curr_chan,scope.channel);
			thisConfig->GetParValueWR(srcparN,val);
			for (it=cset.begin(); it!=cset.end(); ++it){
				sprintf(cpy,"channel%d%s",it->channel,strchr(thisConfig->GetParName(srcparN),'/'));
				printf(" dst = %s\n",cpy);
...
void TGui::on_btn_clicked (GObject *object){
	...
	//"CHIP RESET" button
		printf("____(NOT IMPLEMENTED) Resetting Chip #%d\n",scope.chip);
		//config[scope.chip]->ChipReset();
	}else
	if (btn_name.compare("btn_chanres")==0){
	//"CHANNEL RESET" button
		printf("____(NOT IMPLEMENTED) Resetting Channels of Chip #%d\n",scope.chip);
